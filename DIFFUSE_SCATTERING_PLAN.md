...
lunus is declared on the **linux-only `boltz` feature**, not workspace-level:

```toml
[tool.pixi.feature.boltz]
pypi-dependencies = {..., "lunus" = {rev = "fb22633f...", extras = ["sf"], git = "https://github.com/lanl/lunus.git"}}
platforms = ["linux-64"]
```

so `boltz`, `boltz-dev` and `boltz-analysis` have it and no osx-arm64 environment
does. Three operational notes, each learned the hard way:

- **A workspace-level declaration is unlockable on any machine.** lunus is a git
  source dependency with no published wheel, so pixi must *build* it to read its
  metadata, for every platform an environment declares. macOS fails solving
  linux-64; the Linux runner fails solving osx-arm64 for `boltz-osx`. Neither can
  do both. Confining it to linux-only features is what makes the lock
  producible. **The durable fix is a published wheel** — lunus.sf is pure Python,
  so one `py3-none-any` artifact needs no build anywhere, after which this can
  move back to workspace-level.
- **A rev, not a branch.** A branch pin resolves to a SHA once and then never
  moves: the manifest reads the same, `pixi lock` finds nothing stale, and new
  lunus commits never arrive — the Relock workflow silently committed nothing
  for exactly this reason. Bumping the rev changes the manifest, which is what
  makes a re-lock pick them up.
- **Only ever re-lock with the pinned pixi version.** The Relock workflow pins
  v0.73.0. Other versions rewrite the lock wholesale — 0.76.2 stripped the
  `osx-arm64` entries, 0.71.2 produced a ~9,900-line diff. A lock change that is
  not confined to the dependency you touched is the signal to stop.
