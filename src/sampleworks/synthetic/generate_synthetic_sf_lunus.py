...
def compute_ensemble_amplitudes(
    ...
    *,
    ...
    setup = lunus_sf.build_setup(
        atom_array, unit_cell, space_group, resolution, device=device
    )
    ...
    occupancies = torch.as_tensor(
        np.asarray(atom_array.occupancy, dtype=np.float32), device=device
    )
    ...
    with torch.no_grad():  # generation only; nothing here needs gradients
        f_configs = lunus_sf.structure_factors(
            setup, coords_t, occupancies, hkl, solvent=solvent
        )
...
def parse_args() -> argparse.Namespace:
    ...
    sf_group.add_argument(
        "--remove-hydrogens", action="store_true", help="Single-model input only"
    )
